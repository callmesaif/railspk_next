...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "About", href: "/about" } // Replaced Support with About
  ];
      ...
      <div className={`fixed inset-0 z-[100] ${isOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute right-4 top-4 bottom-4 w-80 bg-white dark:bg-[#0a0f1a] rounded-[3rem] p-10 shadow-2xl transition-transform duration-500 transform ${isOpen ? 'translate-x-0' : 'translate-x-[110%]'}`}>
          ...
          <nav className="flex flex-col space-y-4">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} className="text-2xl font-black italic rounded-3xl p-6 hover:bg-rail-accent/10 hover:text-rail-accent transition-all text-foreground dark:text-white" onClick={() => setIsOpen(false)}>
                {link.name}
              </Link>
            ))}
          </nav>
