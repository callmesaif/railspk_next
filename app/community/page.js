...
export default function CommunityPage() {
    ...
    const [selectedImage, setSelectedImage] = useState(null); // Full Preview State

    // 1. Rich Text Logic
    const formatContent = (text) => {
        if (!text) return "";
        let formatted = text.replace(/#(\w+)/g, '<span class="text-rail-accent font-bold">#$1</span>');
                    ...
                    <div className="space-y-12">
                        ...
                        {updates.map(update => (
                            <div key={update.id} className="bg-white/5 p-8 rounded-[3rem] border border-white/10 shadow-2xl">
                                ...
                                {/* Image Grid with Click-to-Preview */}
