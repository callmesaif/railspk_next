  ...
  return (
    <main className="bg-background text-foreground animate-m3">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-20">
        <div 
          className="absolute inset-0 z-0 m-2 md:m-4 rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden shadow-2xl" 
          style={{ 
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-6 text-center text-white">
          ...
          <div className="bg-white/10 backdrop-blur-2xl border border-white/20 p-4 md:p-5 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl flex flex-col md:flex-row items-center gap-3 md:gap-4 max-w-5xl mx-auto transition-all hover:bg-white/15">
            ...
            <Link href="/reviews" className="w-full md:w-auto px-10 py-4 md:py-5 bg-rail-accent text-white rounded-full font-black uppercase tracking-widest text-[10px] shadow-xl hover:shadow-rail-accent/30 active:scale-95 transition-all">
              Search Reviews
            </Link>
          </div>
      ...
      <section className="py-24 md:py-40 max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
        {[
          ...
          <div key={i} className="m3-card p-10 md:p-12 hover:border-rail-accent/30 group">
            <div className={`w-14 h-14 md:w-16 md:h-16 bg-white dark:bg-[#0a0f1a] rounded-[1.2rem] md:rounded-[1.5rem] flex items-center justify-center ${feat.color} shadow-sm mb-8 md:mb-10 group-hover:bg-rail-accent group-hover:text-white transition-all duration-500`}>
              <span className="material-symbols-rounded text-3xl md:text-4xl">{feat.icon}</span>
            </div>
            <h3 className="text-xl md:text-2xl font-black italic uppercase mb-4 md:mb-5 tracking-tight">{feat.title}</h3>
                  ...
                  >
                    <div className="aspect-video relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-all duration-500">
                        <span className="material-symbols-rounded text-6xl md:text-7xl text-white scale-75 group-hover:scale-100 transition-transform">play_circle</span>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/85 backdrop-blur-md">
          <div className="bg-white dark:bg-[#0a0f1a] p-10 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] max-w-sm w-full text-center border border-white/10 shadow-2xl">
            <div className="w-14 h-14 md:w-16 md:h-16 bg-rail-accent/10 text-rail-accent rounded-full flex items-center justify-center mx-auto mb-6">
              <span className="material-symbols-rounded text-2xl md:text-3xl">exit_to_app</span>
            </div>
            <h3 className="text-lg md:text-xl font-black italic uppercase text-foreground mb-4">Leaving Site</h3>
            ...
            <div className="flex flex-col gap-4">
              <a href={CHANNEL_URL} target="_blank" onClick={() => setShowRedirectPopup(false)} className="bg-rail-accent text-white py-4 md:py-5 rounded-full font-black uppercase tracking-widest text-[10px] md:text-[11px] shadow-xl active:scale-95 transition-all">Confirm & Open</a>
